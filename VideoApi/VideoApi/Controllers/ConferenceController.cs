    ...
    public class ConferenceController : ControllerBase
    {
        ...
        public async Task<IActionResult> BookNewConferenceAsync(BookNewConferenceRequest request)
        {
            ...
            var audioIngestUrl = _featureToggles.HrsIntegrationEnabled()
                ? _audioPlatformService.GetAudioIngestUrl(request.CaseTypeServiceId, request.CaseNumber,
            ...
            var supplierPlatformService =
                _supplierPlatformServiceFactory.Create((Domain.Enums.Supplier)request.Supplier);
            ...
            var response =
                ConferenceToDetailsResponseMapper.MapConferenceToResponse(queriedConference,
                    supplierConfiguration.PexipSelfTestNode);
            
            _logger.LogInformation("Created conference {ResponseId} for hearing {HearingRefId}", response.Id,
        ...
        public async Task<IActionResult> UpdateConferenceAsync(UpdateConferenceRequest request)
        {
            ...
            var conferencesList =
                await _queryHandler.Handle<GetNonClosedConferenceByHearingRefIdQuery, List<Conference>>(query);
            ...
            [FromQuery] ConferenceForAdminRequest request)
        {
            ...
            var conferences =
                await _queryHandler.Handle<GetConferencesTodayForAdminByHearingVenueNameQuery, List<Conference>>(query);
            ...
            [FromQuery] ConferenceForStaffMembertWithSelectedVenueRequest request)
        {
            ...
            var conferences = await _queryHandler
                .Handle<GetConferencesTodayForStaffMemberByHearingVenueNameQuery, List<Conference>>(query);
        ...
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        [ProducesResponseType(typeof(ValidationProblemDetails), (int)HttpStatusCode.BadRequest)]
        public async Task<IActionResult> GetConferencesForAdminByHearingRefIdAsync(
            GetConferencesByHearingIdsRequest request)
        {
            ...
            var response = conferences
                .Select(conference =>
                {
                    ...
                    return ConferenceForAdminResponseMapper.MapConferenceToAdminResponse(conference,
        ...
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        [ProducesResponseType(typeof(ValidationProblemDetails), (int)HttpStatusCode.BadRequest)]
        public async Task<IActionResult> GetConferencesForHostByHearingRefIdAsync(
            GetConferencesByHearingIdsRequest request)
        {
            var query = new GetNonClosedConferenceByHearingRefIdQuery(request.HearingRefIds, true);
            var conferences =
        ...
        public async Task<IActionResult> GetConferencesHearingRoomsAsync([FromQuery] string date)
        {
            ...
            try
            {
                ...
                var interpreterRooms =
                    await _queryHandler.Handle<GetConferenceInterpreterRoomsByDateQuery, List<HearingAudioRoom>>(
        ...
        private async Task<Guid> CreateConferenceAsync(BookNewConferenceRequest request, string ingestUrl)
        {
            ...
            var participants = request.Participants.Select(x =>
                    new Participant(x.ParticipantRefId, x.Name, x.FirstName, x.LastName, x.DisplayName, x.Username,
                        x.UserRole.MapToDomainEnum(), x.HearingRole, x.CaseTypeGroup, x.ContactEmail,
                        x.ContactTelephone)
                    {
        ...
        private async Task<IEnumerable<ConferenceForHostResponse>> GetHostConferencesForToday(string username)
        {
            ...
            var conferenceForHostResponse =
                conferences.Select(ConferenceForHostResponseMapper.MapConferenceSummaryToModel);
        ...
        private async Task DeleteAudioRecordingApplication(Guid conferenceId)
        {
            ...
            if (queriedConference is { AudioRecordingRequired: true } &&
                !queriedConference.IngestUrl.Contains(_audioPlatformService
