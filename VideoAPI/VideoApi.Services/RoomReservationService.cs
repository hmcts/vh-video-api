        ...
        public async Task<Conference> EnsureRoomAvailableAsync(Guid conferenceId, Func<Guid, Task<Conference>> getConferenceAsync)
        {
            var retryPolicy = Policy
                .HandleResult<Conference>(x =>
                {
                    ...
                    {
                        ApplicationLogger.Trace("PRIVATE_CONSULTATION", "EnsureRoomAvailableAsync", $"EnsureRoomAvailableAsync KEY : {reservationKey} : FOUND");
                        return true;
                    }

                    ApplicationLogger.Trace("PRIVATE_CONSULTATION", "EnsureRoomAvailableAsync", $"EnsureRoomAvailableAsync KEY : {reservationKey} : Not FOUND, setting cache");
            ...
            return await retryPolicy.ExecuteAsync(async () => {
                ApplicationLogger.Trace("PRIVATE_CONSULTATION", "EnsureRoomAvailableAsync", $"Conference: {conferenceId} - EnsureRoomAvailableAsync- ExecuteAsync");
